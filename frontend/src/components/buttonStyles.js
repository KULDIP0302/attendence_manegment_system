import styled from 'styled-components';
import { Button } from '@mui/material';

export const RedButton = styled(Button)`
  && {
    background-color: #f00;
    color: white;
    margin-left: 4px;
    font-size: 1rem;
    font-weight: 700;
    padding: 8px 16px;
    border-radius: 10px;
    text-transform: none;
    box-shadow: 0 6px 14px rgba(0, 0, 0, 0.16);
    &:hover {
      background-color: #eb7979;
      border-color: #f26767;
      box-shadow: none;
    }
  }
`;

export const BlackButton = styled(Button)`
  && {
    background-color: #000000;
    color: white;
    margin-left: 4px;
    font-size: 1rem;
    font-weight: 700;
    padding: 8px 16px;
    border-radius: 10px;
    text-transform: none;
    box-shadow: 0 6px 14px rgba(0, 0, 0, 0.16);
    &:hover {
      background-color: #212020;
      border-color: #212020;
      box-shadow: none;
    }
  }
`;

export const DarkRedButton = styled(Button)`
  && {
    background-color: #650909;
    color: white;
    font-size: 1rem;
    font-weight: 700;
    padding: 8px 16px;
    border-radius: 10px;
    text-transform: none;
    box-shadow: 0 6px 14px rgba(0, 0, 0, 0.16);
    &:hover {
      background-color: #eb7979;
      border-color: #f26767;
      box-shadow: none;
    }
  }
`;

export const BlueButton = styled(Button)`
  && {
    background-color: #080a43;
    color: #fff;
    font-size: 1rem;
    font-weight: 700;
    padding: 8px 16px;
    border-radius: 10px;
    text-transform: none;
    box-shadow: 0 6px 14px rgba(16, 35, 115, 0.28);
    &:hover {
      background-color: #0a1e82;
    }
  }
`;

export const PurpleButton = styled(Button)`
  && {
    background-color: #270843;
    color: #fff;
    font-size: 1rem;
    font-weight: 700;
    padding: 8px 16px;
    border-radius: 10px;
    text-transform: none;
    box-shadow: 0 6px 14px rgba(39, 8, 67, 0.28);
    &:hover {
      background-color: #3f1068;
    }
  }
`;

export const LightPurpleButton = styled(Button)`
  && {
    background-color: #7f56da;
    color: #fff;
    font-size: 1rem;
    font-weight: 700;
    padding: 8px 16px;
    border-radius: 10px;
    text-transform: none;
    box-shadow: 0 6px 14px rgba(127, 86, 218, 0.28);
    &:hover {
      background-color: #7a1ccb;
    }
  }
`;

export const GreenButton = styled(Button)`
  && {
    background-color: #133104;
    color: #fff;
    font-size: 1rem;
    font-weight: 700;
    padding: 8px 16px;
    border-radius: 10px;
    text-transform: none;
    box-shadow: 0 6px 14px rgba(19, 49, 4, 0.25);
    &:hover {
      background-color: #266810;
    }
  }
`;

export const BrownButton = styled(Button)`
  && {
    background-color: #2c1006;
    color: white;
    font-size: 1rem;
    font-weight: 700;
    padding: 8px 16px;
    border-radius: 10px;
    text-transform: none;
    box-shadow: 0 6px 14px rgba(44, 16, 6, 0.26);
    &:hover {
      background-color: #40220c;
      border-color: #40220c;
      box-shadow: none;
    }
  }
`;

export const IndigoButton = styled(Button)`
  && {
    background-color: #2f2b80;
    color: white;
    font-size: 1rem;
    font-weight: 700;
    padding: 8px 16px;
    border-radius: 10px;
    text-transform: none;
    box-shadow: 0 6px 14px rgba(47, 43, 128, 0.26);
    &:hover {
      background-color: #534ea6;
      border-color: #473d90;
      box-shadow: none;
    }
  }
`;
